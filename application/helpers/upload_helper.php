...
function get_upload_path_by_type($type)
{
    ...
    switch ($type) {
        ...
        break;
        case 'supplier':
        $path = SUPPLIER_PROFILE_IMAGES_FOLDER;
...
function handle_supplier_profile_image_upload($supplier_id = '')
{
    if (!is_numeric($supplier_id)) {
        $supplier_id = get_supplier_user_id();
    }
    if (isset($_FILES['profile_image']['name']) && $_FILES['profile_image']['name'] != '') {
        hooks()->do_action('before_upload_supplier_profile_image');
        $path = get_upload_path_by_type('supplier') . $supplier_id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['profile_image']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            $allowed_extensions = hooks()->apply_filters('supplier_profile_image_upload_allowed_extensions', $allowed_extensions);
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $config['new_image']      = 'thumb_' . $filename;
                $config['maintain_ratio'] = true;
                $config['width']          = hooks()->apply_filters('supplier_profile_image_thumb_width', 320);
                $config['height']         = hooks()->apply_filters('supplier_profile_image_thumb_height', 320);
                $CI->image_lib->initialize($config);
                $CI->image_lib->resize();
                $CI->image_lib->clear();
                $config['image_library']  = 'gd2';
                ...
                $config['new_image']      = 'small_' . $filename;
                $config['maintain_ratio'] = true;
                $config['width']          = hooks()->apply_filters('supplier_profile_image_small_width', 96);
                $config['height']         = hooks()->apply_filters('supplier_profile_image_small_height', 96);
                $CI->image_lib->initialize($config);
                $CI->image_lib->resize();
                $CI->db->where('supplierid', $supplier_id);
                $CI->db->update(db_prefix() . 'suppliers', [
                    'profile_image' => $filename,
                ]);
                // Remove original image
                unlink($newFilePath);
